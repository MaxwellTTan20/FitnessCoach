...
class _CapybaraCardState extends State<CapybaraCard> {
  ...
  int    get _characterNum   => (_feedCount ~/ _kFeedsPerCharacter).clamp(0, _kTotalCharacters - 1) + 1;
  int    get _xpInLevel      => _feedCount % _kFeedsPerCharacter;
  double get _goldenIntensity => _feedCount < _kGoldenStart
      ? 0.0
      : ((_feedCount - _kGoldenStart) / (_kFullyGolden - _kGoldenStart)).clamp(0.0, 1.0);
  bool   get _isGolden       => _feedCount >= _kGoldenStart;
  double get _levelProgress  => _isGolden ? _goldenIntensity : _xpInLevel / _kFeedsPerCharacter;
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: _isGolden
                              ? Color.lerp(Colors.amber, const Color(0xFFFFD700), _goldenIntensity)!
                              : Colors.amber.withValues(alpha: 0.9),
                          borderRadius: BorderRadius.circular(12),
                        ),
                        child: Text(
                          _feedCount >= _kFullyGolden ? '⭐ MAX' : 'Lv. $_characterNum',
                                    ...
                                    if (intensity > 0) {
                                      img = ColorFiltered(
                                        colorFilter: ColorFilter.mode(
                                          Color.lerp(Colors.white, const Color(0xFFFFD700), intensity)!,
                                          BlendMode.modulate,
                                        ),
                                        child: img,
                                      );
...
class _HeartData {
  ...
  const _HeartData(
      {required this.id, required this.dx, required this.delay});
}
...
class _HeartWidgetState extends State<_HeartWidget>
  ...
  void initState() {
    ...
    _opacity = TweenSequence<double>([
      ...
      TweenSequenceItem(
          tween: Tween(begin: 1.0, end: 1.0), weight: 55),
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.0), weight: 35),
    ]).animate(_ctrl);
    _dy = Tween<double>(begin: 0.0, end: -88.0).animate(
