...
class AppProfile {
  ...
  Future<void> loadFromFirestore() async {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        capybaraFeedCount =
            (d['capybaraFeedCount'] as int?) ?? capybaraFeedCount;
        lifetimeCorrectReps =
            (d['lifetimeCorrectReps'] as int?) ?? lifetimeCorrectReps;
        debugPrint('[Firestore] loaded successfully');
      } else {
        debugPrint(
          '[Firestore] no existing doc — will still attempt migration',
        );
      }
  ...
  Future<void> _migrateLifetimeReps() async {
    ...
    try {
      ...
      debugPrint(
        '[Firestore] migrated lifetimeCorrectReps=$lifetimeCorrectReps, grassBalance=$grassBalance',
      );
